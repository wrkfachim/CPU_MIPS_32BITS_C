...
void UnidadeControle(int IR, int *sc){
	...
	switch(*sc) {
		...
		por sua vez, deve ser implementada representando cada sinal de controle como uma Equação Lógica, considerando a somade-produtos,
		ou seja, implemente em C uma simulação de um Arranjo Lógico Programável (PLA). Devem ser feitas as
		operações bit-a-bit adequadas das entradas e do estado atual da UC para determinar as saídas (sinais de controle). Para o
		controle de sequenciamento use Função Próximo Estado Explícita. Lembre-se: deve haver a implementação em C do que
		seria uma “equação lógica” para cada bit, tanto para sinais de controle quanto para o controle de sequenciamento.
		As instruções a serem implementadas são as 9 já detalhadas em sala de aula (add, sub, slt, and, or, lw, sw, beq e j), e
		também as seguintes: jal, jr, jalr, addi, andi e bne. Devem ser implementadas 15 instruções ao todo.
		Os sinais de controle emitidos pela UC serão representados como bits de uma variável do tipo inteiro (32 bits). A
		ordem, nessa variável de 32 bits, dos sinais de controle já propostos por Patterson & Hennessy para as 9 instruções
		implementadas no livro, além dos sinais criados para que as novas instruções possam ser implementadas, é a seguinte:
